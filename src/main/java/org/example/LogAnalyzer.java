...
public class LogAnalyzer {
    ...
    public static void main(String[] args) throws Exception {
        String endpoint = "https://your-es-endpoint.com";  // <-- set your endpoint
        boolean debug   = false;                            // <-- true → read local file
        int     days    = 1;                               // <-- how many days ago
        ...
        printLargestBySize(indices);
        printLargestByShards(indices);
        printLeastBalanced(indices);
    }
    ...
    public static void printLargestBySize(List<IndexInfo> data) {
        System.out.println("Printing largest indexes by storage size");
        data.stream()
                .sorted(Comparator.comparingDouble(i -> -i.sizeGb))
                .limit(5)
                .forEach(i -> {
                    System.out.printf("Index: %s%n", i.index);
                    System.out.printf("Size: %.2f GB%n", i.sizeGb);
                });
        System.out.println();
    }

    public static void printLargestByShards(List<IndexInfo> data) {
        ...
        data.stream()
                ...
                .forEach(i -> {
                    System.out.printf("Index: %s%n", i.index);
                    System.out.printf("Shards: %d%n", i.shards);
                });
        System.out.println();
    }
    ...
    public static void printLeastBalanced(List<IndexInfo> data) {
        ...
        data.stream()
                ...
                .limit(5)
                .forEach(i -> {
                    // floor for <=2 shards, round for >2 shards
                    ...
                    System.out.printf(
                            "Index: %s%n" +
                                    ...
                                    "Shards: %d%n" +
                                    "Balance Ratio: %d%n" +
                                    "Recommended shard count is %d%n",
                            i.index, i.sizeGb, i.shards, ratio, i.recommendedShards
                    );
                });
    }
